...
- [ ] When ngspice errors out, the "Simulation failed" banner shows the wrapper message ("Invalid ngspice RAW output: missing Variables or Values section") instead of ngspice's actual stderr. Surface the real diagnostic at the top of the panel — the underlying error (singular matrix, undefined source for `.dc`, missing model, etc.) is what tells the user what to fix.
- [ ] Net-label sanitizer collapses distinct user labels into the same SPICE name. `W+` and `W-` both become `W_`, so two unrelated voltage sources end up shorted on the same node, producing a singular matrix with no obvious error. Map `+`/`-` to distinct sequences (e.g. `_p` / `_n`) so different inputs map to different node names, and warn when any sanitizer collision happens.
- [ ] Unify the look of the single-tool tooltip (e.g. `Wire` hover card) and the grouped-tool hover-submenu (e.g. `Sources` popover). They currently differ in panel padding, header weight/size, body text styling, arrow shape, and shadow — so they read as two different UI primitives even though they serve the same role. Pick one design language (probably the popover's — bigger card, clear header, list of items) and have the single-tool hover render in the same shell, just with one row.
