...
- Real-circuit browser smoke tests now cover rectifier, RLC filter, CMOS inverter, subcircuit, op-amp, BJT, NMOS differential pair, NMOS common-source amplifier, and ReLU learning-cell documents rather than the examples menu. The inverter fixture caught an accidental input/output short caused by a routing path through a MOS drain/output pin, and the differential-pair fixture keeps explicit junction points at pass-through pins so the test mirrors how users should draw connected rails. The common-source fixture adds a single-transistor biased gain stage with labeled gate, drain, and source nodes. The corrected fixtures now validate intended labeled traces while keeping generated nodes and current vectors hidden by default.
